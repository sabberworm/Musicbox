var spawn = require('child_process').spawn;
var matcher = /("(.+)"|(\b(.+)\b))/;

var parse_number = function(str) {
	var result = parseInt(str, 10);
	if(isNaN(result)) {
		return null;
	}
	return result;
};

function unicode_clean(str) {
	var matches = str.match(/\\U([\da-fA-F]){4}/g);
	if(matches !== null) {
		matches.forEach(function(match) {
			// FIXME: Find better solution for this…
			// 1. eval is slow
			// 2. eval’d string needs extra protection
			// 3. replace() is slower than it needs to be because actual location of the match should be known
			var chr = eval('"'+match.toLowerCase()+'"');
			str = str.replace(match, chr);
		});
	}
	return str;
}

module.exports = function(global, options) {
	var opts = options.indexers.mdls || {executable: "mdls"};
	var executable = opts.executable;

	var md_for_file = function(file, type, error_handler, callback, hard_error_on_null) {
		var proc;
		try {
			proc = spawn(executable, ['-name', type, '-raw', file]);
		} catch(e) {
			setTimeout(arguments.callee, 1000*(Math.random()+1), file, type, callback);
			return;
		}
		var str = '';
		proc.stdout.on('data', function(data) {
			str += data.toString('utf-8');
		});
		proc.on('exit', function(code, signal) {
			str = unicode_clean(str)
			if(str === '(null)') {
				str = null;
			}
			if(code !== 0 || (str === null && hard_error_on_null)) {
				return error_handler(file, signal);
			}
			callback(str);
		});
	};

	return {
		index: function(file, insert, error_handler) {
			try {
				var metadata = {};
				md_for_file(file, 'kMDItemTitle', error_handler, function(md) {
					metadata.name = md;
					md_for_file(file, 'kMDItemAlbum', error_handler, function(md) {
						metadata.album = md;
						md_for_file(file, 'kMDItemAuthors', error_handler, function(md) {
							metadata.artist = md.match(matcher);
							if(metadata.artist) {
								metadata.artist = metadata.artist[2] || metadata.artist[3];
							}
							md_for_file(file, 'kMDItemAudioTrackNumber', error_handler, function(md) {
								metadata.track_number = parse_number(md);
								md_for_file(file, 'kMDItemRecordingYear', error_handler, function(md) {
									metadata.year = parse_number(md);
									md_for_file(file, 'kMDItemMusicalGenre', error_handler, function(md) {
										metadata.genre = md;
										insert(file, metadata);
									});
								});
							});
						}, true);
					});
				}, true);
			} catch (e) {
				error_handler(file, e);
			}
		}
	};
};