...
var dirs = [];
var extensions = [];
var finder = new Finder().type(Finder.FILE).nameMatchMode(Finder.MODE_ANY);
			...
			if(track == null) {
				...
				for(var field in metadata) {
					if(track[field] !== metadata[field]) {
						...
						has_updated = field !== 'has_been_updated' && field !== 'last_update';
					}
				}
				if(has_updated) {
	...
	finder.fetch(dirs, function(error, files) {
		...
		files.forEach(function(file) {
			if(running_indexers > max_indexers) {
				setTimeout(arguments.callee, 50, file);
				return;
			}
