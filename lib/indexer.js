...
var finalize = function() {
	...
	tracks.remove({has_been_updated: false}, function(error) {
		...
		log('debug', "Removed unupdated items");
		tracks.update({has_been_updated: true}, {"$set": {has_been_updated: false}}, {multi: true}, function(error) {
			if(error) {
				log('error', "Error when setting has_been_updated to false", error);
			}
			log('status', "Finished indexer run, tagged", items_tagged, "items with", items_error, "errors");
			items_tagged = 0;
			items_error = 0;
...
var indexer_error = function(path, error) {
	log('error', "Indexer error", error, "for file", path);
	items_error++;
	ran_indexer();
};
...
var insert = function(path, metadata) {
	tracks.findOne({path: path}, function(error, track) {
		...
		if(track == null) {
			...
			log('info', "Inserting track:", metadata.artist, "–", metadata.name);
			...
			if(has_updated) {
				log('debug', "Updating track:", track.artist, "–", track.name);
			} else {
				// log('debug', "Track identical:", track.artist, "–", track.name)
			}
			tracks.save(track);
		}
		ran_indexer();
...
index_task = function() {
	finder.fetch(dirs, function(error, files) {
		log('status', "Started indexer, tagging", files.length, "items");
